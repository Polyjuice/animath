...
// Create anchors
const anchors = ANCHOR_POSITIONS.map(pos => {
    const anchor = new Particle(vec2fn, { x: pos.x, y: pos.y }, 0);
    composite.particles.push(anchor);
    return anchor;
});
// Create draggable point at center
const dragPoint = new Particle(vec2fn, { x: centerX, y: centerY }, 1);
composite.particles.push(dragPoint);
// Create breakable constraints from drag point to each anchor
const springs = [];
anchors.forEach((anchor, i) => {
    const spring = new Constraint(vec2fn, anchor, dragPoint, 0.8);
...
artboard.addEventListener("mousedown", (e) => {
    ...
    const dist = Math.sqrt(Math.pow(mouseX - dragPoint.pos.x, 2) +
...
function getStrainColor(strain, broken) {
    if (broken)
        return "#ff4444";
...
function drawFrame(time) {
    ...
    springs.forEach((spring, i) => {
        ...
        ctx.fillText(spring.broken ? "BROKEN" : `${(spring.currentStrain * 100).toFixed(0)}%`, midX + offsetX, midY + offsetY);
    });
    // Draw anchors
    anchors.forEach((anchor, i) => {
        ctx.beginPath();
...
requestAnimationFrame(drawFrame);
//# sourceMappingURL=rubber-band-demo.js.map