...
function createVertices(n, width, height) {
    return Array(n).map(function (e, i, self) {
        ...
        var vert = new Particle(vec2fn, vec2add(vec2create(), rot, vec2(width / 2, height / 2)));
...
function createConstraints(vertices) {
    ...
    return points.map(function (p) {
        return new Constraint(vec2fn, p[0], p[1], 0.05, 10);
    });
}
...
// }
//# sourceMappingURL=line-demo.js.map