 ...
 */
export class CPhysicsSimulator {
    settings;
    random = randomer(42);
    ...
    integrate;
    createBody;
    totalMovement = 0; // how much movement we made on last step
    forces = [];
    forceMap = new Map();
    iterationNumber = 0;
    dirty = false;
    ...
    constructor(settings) {
        ...
        this.createBody = (pos) => new Body(pos);
    ...
    addForce(forceName, forceFunction) {
        if (this.forceMap.has(forceName))
     ...
     */
    addSpring(body1, body2, springLength, springCoefficient = 1) {
        if (!body1 || !body2) {
...
export class Spring {
    ...
    data;
    unreal = false;
    constructor(from, to, length, coefficient, data) {
        this.from = from;
        this.to = to;
        this.length = length;
