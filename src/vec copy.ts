...
export function vec2solve(particles: Particle<vec2>[], constraints: Constraint<vec2>[], barriers: Constraint<vec2>[], iterations: number, attraction: number): number {
    let k = (hooksK / iterations);
    let f = 1 - (friction / iterations);
    let m = mass * iterations;
    ...
    for (let i = 0; i < iterations; i++) {
        ...
        for (const s of constraints) {
            ...
            let sx = dx * stretch;
            let sy = dy * stretch;
            let p0accx = -sx / (p0mass * m);
            let p0accy = -sy / (p0mass * m);
            let p1accx = +sx / (p1mass * m);
            let p1accy = +sy / (p1mass * m);
        ...
        for (const b of barriers) {
            ...
            for (const p of particles) {
                let before = p.pos;
                let vel = p.velocity;
                let includingradius = { x: p.pos.x + vel.x, y: p.pos.y + vel.y };
                vec2grow(includingradius, before, includingradius, 3);
                vec2collide2(vel, start, end, before, includingradius);
            }
        }
        ...
        for (const p of particles) {
            ...
            maxvely = Math.max(maxvely, vely);
        }
    }

    return vec2dist({ x: 0, y: 0 }, { x: maxvelx, y: maxvely })
}

export function vec1solve(particles: Particle<vec1>[], constraints: Constraint<vec1>[], barriers: Constraint<vec1>[], iterations: number): number {
    return 0;
}
export function vec3solve(particles: Particle<vec3>[], constraints: Constraint<vec3>[], barriers: Constraint<vec3>[], iterations: number): number {
    return 0;
}
...
function vec2reflection(out: vec2, incomming: vec2, normal: vec2): vec2 {
    ...
    let par = 2 * vec2dot(incomming, normal);
    vec2scale(out, normal, par);
    vec2sub(out, incomming, out);
    return out;
}

function vec2surfacenormals(out1: vec2, input: vec2): void {
    let inpn = vec2create();
    vec2normalize(inpn, input);
...
function vec2collide2(velocity: vec2, wallstart: vec2, wallend: vec2, particlestart: vec2, particleend: vec2): boolean {
    let v0 = ccw(wallstart, particlestart, particleend);
    let v1 = ccw(wallend, particlestart, particleend);
    let before = ccw(wallstart, wallend, particlestart);
    let after = ccw(wallstart, wallend, particleend);
    let diff0 = (v0 > 0) != (v1 > 0);
    let diff1 = (before > 0) != (after > 0);
    if (diff0 && diff1) {
...
vec2sub(floor, floor2, floor1);
vec2surfacenormals(floornormal, floor);
let ray = { x: 10, y: -10 };
