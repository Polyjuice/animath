...
import { vec1, vec1fn, vec1fromValues, vec2, vec2create, vec2fn, vec2normalize, vec2scale, vec2sub, vec3, vec3fn } from "../src/vec.js"
 ...
 */
export function createSpringChain1D(origin: number, spacing: number, segments: number, stiffness: number, mass: number = 1): Composite<vec1> {
    ...
    for (let i = 0; i < segments; i++) {
        ...
        composite.particles.push(new Particle<vec1>(vec1fn, vec1fromValues(x), particleMass));
    }

    // Create constraints between adjacent particles
    for (let i = 0; i < segments - 1; i++) {
        composite.constraints.push(
            new Constraint<vec1>(vec1fn, composite.particles[i], composite.particles[i + 1], stiffness)
        );
    }
 ...
 */
export function createAnchoredSpring1D(anchorX: number, targetX: number, stiffness: number, mass: number = 1): Composite<vec1> {
    const composite = new Composite<vec1>();
    composite.name = "anchored-spring-1d";

    const anchor = new Particle<vec1>(vec1fn, vec1fromValues(anchorX), 0); // mass=0 means fixed
    const target = new Particle<vec1>(vec1fn, vec1fromValues(targetX), mass);

    composite.particles.push(anchor);
    composite.particles.push(target);

    composite.constraints.push(
        new Constraint<vec1>(vec1fn, anchor, target, stiffness)
    );
 ...
 */
export function createSpringComparison1D(startX: number, length: number, count: number, stiffnesses: number[]): Composite<vec1>[] {
    const composites: Composite<vec1>[] = [];
 ...
 * @param stiffness Spring stiffness
 */
export function createSoftCube3D(origin: vec3, size: number, stiffness: number): Composite<vec3> {
    const composite = new Composite<vec3>();
    ...
    for (const v of vertices) {
        composite.particles.push(new Particle<vec3>(vec3fn, v, 1));
    }
    ...
    for (const [a, b] of edges) {
        composite.constraints.push(
            new Constraint<vec3>(vec3fn, composite.particles[a], composite.particles[b], stiffness)
        );
    }
    ...
    for (let z = 0; z < segmentsZ; z++) {
        for (let y = 0; y < segmentsY; y++) {
            for (let x = 0; x < segmentsX; x++) {
                ...
                composite.particles.push(new Particle<vec3>(vec3fn, pos, 1));
            }
        }
    }

    // Helper to get particle index
    const idx = (x: number, y: number, z: number) => z * segmentsY * segmentsX + y * segmentsX + x;
    ...
    for (let z = 0; z < segmentsZ; z++) {
        for (let y = 0; y < segmentsY; y++) {
            for (let x = 0; x < segmentsX; x++) {
                ...
                if (x < segmentsX - 1) {
                    composite.constraints.push(
                        new Constraint<vec3>(vec3fn, composite.particles[current], composite.particles[idx(x + 1, y, z)], stiffness)
                    );
                }
                ...
                if (y < segmentsY - 1) {
                    composite.constraints.push(
                        new Constraint<vec3>(vec3fn, composite.particles[current], composite.particles[idx(x, y + 1, z)], stiffness)
                    );
                }
                ...
                if (z < segmentsZ - 1) {
                    composite.constraints.push(
                        new Constraint<vec3>(vec3fn, composite.particles[current], composite.particles[idx(x, y, z + 1)], stiffness)
                    );
                }
            }
        }
 ...
 */
export function createChain3D(start: vec3, end: vec3, segments: number, stiffness: number): Composite<vec3> {
    const composite = new Composite<vec3>();
    ...
    for (let i = 0; i < segments; i++) {
        ...
        const mass = i === 0 ? 0 : 1;
        composite.particles.push(new Particle<vec3>(vec3fn, pos, mass));
    }

    for (let i = 0; i < segments - 1; i++) {
        composite.constraints.push(
            new Constraint<vec3>(vec3fn, composite.particles[i], composite.particles[i + 1], stiffness)
        );
    }
